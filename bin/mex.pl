...
sub do_setup
{
    ...
    if ($main::mbuild eq 'no' && !$setupFailure) {
    ...
    else
    {
        ...
        {            
            if(tool_name() eq "mbuild"){
                 ...
                 expire("Error: mex.pl No compiler options file could be found\n" .
                      "  to compile source code. Please run \"mbuild -setup\" to rectify.");
            }
...
sub parse_common_dash_args
{
    ...
    ARGTYPE: { 
      ...
      /^-outdir$/ && do {
        ...
        # OUTDIRN makes Intel compilers not work on I18N machines.
        $ENV{'OUTDIRN'} = mexCanonpath($outdir);
...
sub fix_mbuild_variables
{
    ...
    #
    my $simulink_inc_dir = mexCatdir($ML_DIR, "simulink", "include");
    if (-e remove_quotes($simulink_inc_dir))
    {
        $FLAGS = "-I$simulink_inc_dir $FLAGS";
    }
